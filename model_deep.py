...
def my_model():
    ...
    model.add(Dropout(0.4))
            ...
            # 3em Convolution
    model.add(Convolution3D(filters=128, kernel_size=2, padding='valid',
                            data_format='channels_first',))
    model.add(LeakyReLU(alpha = 0.1))
    model.add(MaxPooling3D(pool_size=(2,2,2), strides=None, padding='valid',
                            data_format='channels_first'))
...
def training(model , x_train , y_train , x_test , y_test):
    ...
    EarlyStopping(monitor='val_loss',min_delta=0.1,patience=6,verbose=2,mode='auto',
    baseline=None, restore_best_weights=False)
    model.fit(x_train, y_train, validation_data=(x_test, y_test), epochs=1, batch_size = 50)
